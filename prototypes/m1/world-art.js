...
const WorldArt=(()=>{
 ...
 // Background hills form a skyline without blocking the actual course.
 for(let i=0;i<28;i++){const m=mesh(new T.SphereGeometry(1,16,10),i%2?'#8ba596':'#78968c',[-100+i*67,-35,-430-rnd()*100],[90+rnd()*90,65+rnd()*60,100+rnd()*70],decor);m.rotation.y=rnd()*6;}
 ...
 scene.background=new T.Color('#c1d3cf');scene.fog.color.set('#c1d3cf');scene.children.filter(o=>o.isHemisphereLight).forEach(o=>{o.color.set('#d9eef5');o.groundColor.set('#685947');o.intensity=1.5;});sun.color.set('#ffe0ad');sun.intensity=3.1;renderer.toneMappingExposure=1.02;
 environment.traverse(m=>{if(m.isMesh&&m.material.map&&m.material.color.getHexString()==='397a89')water.push(m.material.map);});
 return {update(dt,reduced){if(!reduced)clock+=dt;sky.position.copy(camera.position);skyMat.uniforms.time.value=clock;for(const map of water)map.offset.x=clock*.004;},stats:{components:packed.userData.rawParts,batches:packed.children.length}};
})();
