...
const WorldArt=(()=>{
 ...
 const landscape=new T.Mesh(new T.PlaneGeometry(4400,2400),new T.MeshStandardMaterial({map:grassMap,color:'#b4c598',roughness:1}));landscape.rotation.x=-Math.PI/2;landscape.position.set(1050,-.5,0);landscape.receiveShadow=true;scene.add(landscape);
 ...
 const grassBlades=new T.InstancedMesh(bladeGeo,grassMaterial,2800),dummy=new T.Object3D();
 const verge=x=>x<25?25:x<94?39:x<211?51:x<410?84:x<650?119:x<1020?165:x<1530?208:x<2120?252:312;
 for(let i=0;i<2800;i++){const x=-70+rnd()*2960,z=(rnd()>.5?1:-1)*(verge(x)+rnd()*70);dummy.position.set(x,-.48,z);dummy.rotation.set(0,rnd()*Math.PI,.12*(rnd()-.5));const s=.55+rnd()*.9;dummy.scale.set(s,s,s);dummy.updateMatrix();grassBlades.setMatrixAt(i,dummy.matrix);grassBlades.setColorAt(i,new T.Color(i%3?'#789a55':'#a0ad5f'));}grassBlades.receiveShadow=true;scene.add(grassBlades);
 ...
 for(const side of [-1,1])for(let i=0;i<45;i++){const m=mesh(new T.SphereGeometry(1,18,11),i%3?'#8ba596':'#718e82',[-120+i*70,-38,side*(470+rnd()*140)],[100+rnd()*105,68+rnd()*70,105+rnd()*85],decor);m.rotation.y=rnd()*6;}
