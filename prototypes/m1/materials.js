...
const ScrapSurfaces=(()=>{
 ...
 function material(color,metal=.1,kind=infer(color,metal),repeat=[1,1]){
  ...
  const map=texture(kind).clone();map.repeat.set(...repeat);map.needsUpdate=true;const roughness={glass:.12,steel:.3,paint:.48,wood:.87,rubber:.97,brick:.95,concrete:.94,asphalt:1}[kind];
