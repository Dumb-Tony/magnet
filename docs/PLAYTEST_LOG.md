...
## 13 September 2026 — Railworks and dry docks

Expanded the active course to six districts, 790 objects and 34 types. New route milestones: locomotive and cargo freighter. Added eight optional Field notes goals, travel to unlocked districts, ten modeled object types and nine additional crushed variants. Original 490 IDs remain stable; completed-city saves advance into the railworks.

Final automated normal and digital routes both reached all six milestones. Local Chrome 153 frame samples averaged 12.63 ms (normal, max 20.1) and 10.17 ms (digital, max 15.1), with different pile compositions. The 600-second stress run reached 310 attached objects and completed without invalid state. All model, crush, storage and expansion-specific regressions passed with zero JavaScript errors. The digital replay's original fixed travel timeout was too short for a 283 m crossing; its budget now scales with distance, without skipping movement or teleporting the route.

Expansion fixtures verify legacy layout IDs, completed-city save migration, locked travel rejection, retained pile transforms during travel, ejected objects beyond x=650, objective deduplication and note restoration. Visually inspected the new vehicles, ships, railworks, docks and notes panel. Automated testing and rendered screenshot review only; no fresh human feel-test claim.

## 13 September 2026 — World graphics and Meridian Airfield

Expanded to seven districts, 967 objects, 40 types and 28 crushed variants. Added workshop structure, street surfacing, facade details, landscaping, atmospheric sky, dock and runway detail, plus a High-graphics HDR/depth-shading pass with direct-render fallback. Static decorative components are combined from 4,941 parts into 79 material batches. The airfield adds six collectible types and the final airliner milestone.
...
## 14 September 2026 — Large-pile pace and environmental motion

Raised the base travel speed and added capped power and rolling-radius contributions, producing representative cruise speeds from 7.66 units/second at the bare core to 27.68 at launch scale. Large-pile response eases modestly from 9/s toward 7/s so the assembly retains weight. Replaced span-only camera pullback with a restrained span-plus-radius curve. A representative launch pile now retains 42% of the bare core's screen-space pace, up from about 23% under the previous curve. Added a lightweight vertex sway to all 2,200 instanced grass blades; Reduced motion freezes it.

The new movement-scaling browser check passed all four representative sizes, including settled velocity, two-second acceleration, camera distance and visible-pace bounds. The complete eight-district movement replay reached every milestone with 870 attachments, no recovery and no page errors, then remained finite through another 60 simulated seconds. Its 120-frame local headless Chrome sample averaged 20.42 ms and peaked at 25.2 ms at 1440 × 900 on High graphics. The graphics regression also passed High/Low switching, resizing and shader checks. These are automated replay and rendering measurements; the user's next play session remains the feel test.

## 16 September 2026 — Titan Foundry

Appended a ninth 700-metre district after the orbital launch complex without changing the original 1,116 object IDs. The Titan Foundry adds 181 objects, five collectible types, five permanent crushed variants and one Field note. Current totals are 1,297 objects, 50 types, 38 crushed variants and nine milestones. The environment adds furnace halls, ore vessels, truss pipe bridges, smokestacks, slag channels and hazard walkways. The landscape and wind-swept grass now extend across the longer course with 2,800 instanced blades.

Normal and keyboard-quantized complete movement replays reached the blast furnace with 891 and 864 attachments, all nine milestones, no recovery and no page errors, then remained finite for another 60 simulated seconds. Reducing the buried-interior visual cap from 440 to 360 kept 334 exterior/recent meshes in the 520-piece density fixture. Combining 6,520 static scenery components into 243 material batches further reduced the final normal replay to 4,387 render calls. Its 120-frame local headless Chrome sample averaged 16.50 ms and peaked at 20.1 ms at 1440 × 900 on High graphics. Every hidden item still retained collision, mass and its saved transform. The dedicated foundry check verified the original-ID prefix, completed-rocket save migration, 260-pixel map layout, reachable final power, all five deformations and the fixed core. All 50 gallery models passed finite bounds, batching and cache checks; all 38 crushed profiles reduced height and volume. Automated replay and screenshot inspection only; no fresh human feel-test claim.

## 22 September 2026 — Quarry, hydro and megacity

Appended Crown Quarry, Halcyon Hydro and Apex Core while preserving the first 1,297 object IDs. The course now has 12 milestones, 1,770 objects, 62 types, 50 crushed profiles and 12 Field notes. New scenery includes terraced quarry walls and conveyors, a reservoir and spillway complex, and a vertical skyline around an open central avenue. Late-game speed gains a capped extra contribution above power 70, while camera pullback now limits the influence of unusually long protrusions.

The local complete replay reached all twelve goals with 1,191 attached objects and no recovery or page errors, then remained finite for another 60 simulated seconds. Its 120-frame Chrome sample averaged 14.21 ms and peaked at 20.2 ms at 1440 × 900 on High graphics; the longest simulation step measured 1.4 ms. All 62 gallery models had finite bounds, reused geometry and batched their components. All 50 crushed forms reduced both height and volume. The 520-piece density audit, exact save/reload, storage-denied play and four-size movement scaling checks also passed. Automated replay and rendered inspection only; no fresh human feel-test claim.

## 24 September 2026 — World-scale escalation

Appended Leviathan Coast, the Orbital Ringworks and the World Engine while preserving the first 1,770 object IDs. The active build now has 15 districts, 2,157 objects, 74 types, 62 crushed profiles and 15 Field notes. Late-game travel and camera pullback gain additional capped scaling beyond power 150 so the 200-metre pile remains navigable and visible.

The complete local replay reached all fifteen milestones with 1,499 objects attached before the final goal, no recovery and no page errors. After another 60 simulated seconds the pile remained finite at power 521.79 with a 0.32 m core; the 120-frame sample averaged 13.96 ms and peaked at 20 ms. All 74 gallery models passed finite bounds, component batching and geometry-cache checks. All 62 crushed forms reduced height and volume. Exact save/reload, storage-denied play, legacy migration and movement-scaling checks passed. Automated replay and rendered inspection only; no fresh human feel-test claim.
