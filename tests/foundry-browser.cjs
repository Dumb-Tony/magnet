...
(async()=>{
 ...
 assert.equal(report.migration.stage,8);assert.equal(report.migration.total,1297);assert.equal(report.regions,9);assert.equal(report.mapWidth,260);assert.equal(report.foundry,181);assert.equal(report.goal,'blastfurnace');assert(report.reachable);assert.equal(report.result.state,'result');assert.equal(report.result.coreRadius,.32);assert.equal(report.notes.contracts.length,9);
